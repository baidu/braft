...
//DECLARE_bool(raft_enable_single_stable_storage);
//DECLARE_bool(raft_enable_merged_stable_storage);
...
TEST_F(TestUsageSuits, mixed_stable_storage) {
    ...
    // test _merged_impl catch up data when first load
    /*
    {
        // _merged_impl has no data at first
        int64_t term_bak = 0;
        braft::PeerId peer_bak;
        st = tmp->_merged_impl->get_term_and_votedfor(&term_bak, &peer_bak, v_group_id);
        ASSERT_FALSE(st.ok());
        ASSERT_EQ(ENOENT, st.error_code());
        ASSERT_EQ(0, term_bak); 
        ASSERT_EQ(braft::ANY_PEER, peer_bak);
    }
    */
    {
        // _merged_impl catch up data when first load
        int64_t term_bak;
        braft::PeerId peer_bak;
        st = storage->get_term_and_votedfor(&term_bak, &peer_bak, v_group_id);
        ASSERT_TRUE(st.ok());
    ...
    {
        // _merged_impl already catch up data after first load
        int64_t term_bak;
        braft::PeerId peer_bak;
        st = tmp->_merged_impl->get_term_and_votedfor(&term_bak, &peer_bak, v_group_id);
        ASSERT_TRUE(st.ok());
