...
public:
    MockFSM(const braft::EndPoint& address_)
        : address(address_)
        , applied_index(0)
        , snapshot_index(0)
...
public:
    ...
    int start(const braft::EndPoint& listen_addr, bool empty_peers = false,
    ...
    int stop(const braft::EndPoint& listen_addr) {
    ...
    void stop_all() {
        std::vector<braft::EndPoint> addrs;
    ...
    void clean(const braft::EndPoint& listen_addr) {
    ...
    void ensure_leader(const braft::EndPoint& expect_addr) {
CHECK:
        std::lock_guard<raft_mutex_t> guard(_mutex);
        for (size_t i = 0; i < _nodes.size(); i++) {
...
private:
    void all_nodes(std::vector<braft::EndPoint>* addrs) {
        addrs->clear();
    ...
    braft::Node* remove_node(const braft::EndPoint& addr) {
        std::lock_guard<raft_mutex_t> guard(_mutex);
    ...
    std::map<braft::EndPoint, brpc::Server*> _server_map;
...
TEST_P(NodeTest, InitShutdown) {
    ...
    braft::Node node("unittest", braft::PeerId(braft::EndPoint(butil::my_hostname(), 5006), 0));
...
TEST_P(NodeTest, SingleNode) {
    ...
    braft::PeerId peer;
    peer.addr.hostname = butil::my_hostname();
    peer.addr.port = 5006;
    peer.idx = 0;
...
TEST_P(NodeTest, NoLeader) {
    ...
    // add peer1
    braft::PeerId peer3;
    peer3.addr.hostname = butil::my_hostname();
    ...
    // remove peer1
    braft::PeerId peer0;
    peer0.addr.hostname = butil::my_hostname();
...
TEST_P(NodeTest, TripleNode) {
...

    {
        ...
        if (channel.Init(leader->node_id().peer_id.addr.to_string().c_str(), &options) != 0) {
...
TEST_P(NodeTest, JoinNode) {
    ...
    // start peer1
    braft::PeerId peer1;
    peer1.addr.hostname = butil::my_hostname();
    ...
    braft::PeerId peer2;
    peer2.addr.hostname = butil::my_hostname();
...
TEST_P(NodeTest, SetPeer1) {
    ...
    boot_peer.addr.hostname = butil::my_hostname();
