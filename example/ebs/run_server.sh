...
DEFINE_integer block_num '32' "Number of blocks"
...
for ((i=0; i<$FLAGS_server_num; ++i)); do
    raft_peers="${raft_peers}${IP}:$((8400+i)):0,"
done
...
for ((i=0; i<$FLAGS_server_num; ++i)); do
    ...
    ${VALGRIND} ./block_server ${HAS_VALGRIND} \
        ...
        -sync_data=${FLAGS_sync_data} \
        -raft_use_fsync_rather_than_fdatasync=true \
        -block_num=${FLAGS_block_num} \
        -ip_and_port="0.0.0.0:$((8400+i))" -peers="${raft_peers}" > std.log 2>&1 &
    cd ../..
done
