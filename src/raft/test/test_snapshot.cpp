...
TEST_F(TestUsageSuits, writer_and_reader) {
    ...
    raft::SnapshotWriter* writer = storage->create(meta);
    ASSERT_TRUE(writer != NULL);
    ASSERT_EQ(0, writer->init());
    ASSERT_EQ(0, writer->save_meta());
    ASSERT_EQ(0, storage->close(writer));

    // normal create writer again
    meta.last_included_index = 2000;
    meta.last_included_term = 2;
    writer = storage->create(meta);
    ASSERT_TRUE(writer != NULL);
...
TEST_F(TestUsageSuits, copy) {
    ...
    ASSERT_EQ(0, raft::init_raft("0.0.0.0:60006", &server, NULL));
    ...
    // normal create writer
    raft::SnapshotWriter* writer1 = storage1->create(meta);
    ASSERT_TRUE(writer1 != NULL);
    ASSERT_EQ(0, writer1->init());
    ASSERT_EQ(0, writer1->save_meta());
    ASSERT_EQ(0, storage1->close(writer1));

    raft::SnapshotReader* reader1 = storage1->open();
    ASSERT_TRUE(reader1 != NULL);
    ASSERT_EQ(0, reader1->init());
    ...
    raft::SnapshotWriter* writer2 = storage2->create(meta);
    ASSERT_TRUE(writer2 != NULL);
    ASSERT_EQ(0, writer2->init());
