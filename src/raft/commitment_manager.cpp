...
CommitmentWaiter::CommitmentWaiter()
    : _max_uncommited_size(1024)
    , _waiter(NULL)
{
...
int CommitmentManager::init(const CommitmentManager &options) {
    if (options.waiter == NULL) {
        LOG(ERROR) << "waiter is NULL";
        return EINVAL;
    }
    const size_t spaces_size = sizeof(void*) * options.max_pending_size;
    void *queue_spaces = malloc(spaces_size);
    if (queue_spaces == NULL) {
        LOG(ERROR) << "Fail to malloc spaces_size=" << spaces_size;
        return errno;
    }
    BoundedQueue<void*> tmp_queue(queue_spaces, spaces_size, base::OWNS_STORAGE);
...
int CommitmentManager::set_stable_at_peer_reentrant(
    ...
    if (log_index >= _pending_index + _pending_apps.size()) {
        CHECK(false);
        return ERANGE;
    }
    PendingMeta *pm = *_pending_apps.top(log_index - _pending_index);
    std::set<PendingMeta*>::iterator iter = pm->peers.find(peer);
    ...
    void* saved_contex = pm->contex;
    delete pm;
    _pending_apps.pop();
    CHECK(_pending_index == log_index);  // FIXME:
    ++_pending_index;
    _last_commited_index = log_index;
    _waiter->on_committed(log_index, saved_contex); // FIXME
    return 0;
}
...
int CommitmentManager::clear_pending_applications() {
    ...
    for (size_t i = 0; i < _pending_apps.size(); ++i) {
        ...
        _pending_apps.pop();
        _waiter->on_cleared(_pending_index + i, pm->contex);
        delete pm;
    }
    _pending_index = 0;
    return 0;
...
int CommitmentManager::reset_pending_index(size_t new_pending_index) {
    BAIDU_SCOPED_LOCK(&_mutex);
